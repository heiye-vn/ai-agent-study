...
// PromptTemplate：负责吧 context / question 拼成最终 prompt
...
const buildPromptInput = new RunnableLambda({
  func: async (input: any) => {
    ...
    retrievedContent.forEach((item: any, i: number) => {
...
// 组合完整的 RAG Runnable（检索 -> 构建 prompt 输入 -> PromptTemplate -> LLM -> 文本）
