...
const routeQuestionNode = async (state: IState) => {
  ...
  const router = getLlm().withStructuredOutput(RouteSchema, {
    method: 'jsonMode',
  });
  const route = await router.invoke(`
你是问答路由器。请判断用户问题是否需要外部检索。
你必须返回一个 JSON 对象（不要包裹在数组中），结构必须精确如下：
{
...
const evaluateNode = async (state) => {
  ...
  const evaluator = getLlm().withStructuredOutput(EvaluateSchema, {
    method: 'jsonMode',
  });
...
{
  "enough": true 或 false,
  "missing": ["缺失信息点1", "缺失信息点2", ...],
  "reason": "简短原因"${hasWeb ? '' : ',\n  "web_query": "联网搜索的查询句，没有则为空"'}
}
...
async function bochaWebSearch(query, count) {
  const apiKey = envVars.BOCHA_API_KEY;
  if (!apiKey) {
    throw new Error('Bocha Web Search 的 API Key 未配置（环境变量 BOCHA_API_KEY）。');
  }
  const url = envVars.BOCHA_REQ_URL || 'https://api.bochaai.com/v1/web-search';
  const body = {
    query,
    freshness: 'noLimit',
    summary: true,
...
const generateNode = async (state) => {
  ...
  process.stdout.write('\n【AI 回答（流式）】\n');
  let generation = '';
  const stream = await getLlm().stream(`你是一个严谨的中文问答助手。优先依据上下文作答，不要编造。
