...
import { webSearch } from './tools/01-search.js';
...
const orchestratorPrompt = dedent`
  ...
  3. **分析** — 调研完成后，若任务涉及任何数值计算（求和、占比、排名、同比增速、均值等），**必须**委派分析师子 Agent 完成。主 Agent 禁止自行做数值计算——所有数字处理交给 analyst 在 REPL 中执行，确保结果可复现
  ...
  **仅**以下 subagent_type 合法：researcher、analyst、editor、general-purpose。
  ...
  - 任务中出现以下关键词时**必须**委派分析师（analyst）：计算、求和、总和、占比、比重、排名、增速、增长率、同比、环比、均值、平均。主 Agent 自身不做任何数值运算
  - 每份报告只调用编辑一次（草稿完成后）
...
export function createIntelligenceDeskAgent() {
  const apiKey = envVars.QWEN_API_KEY.trim();
  if (!apiKey) {
    throw new Error('未设置 OPENAI_API_KEY 环境变量');
  }

  const model = envVars.QWEN_MODEL_NAME?.trim() || 'gpt-4o';
  const baseURL = envVars.QWEN_BASE_URL?.trim() || undefined;
