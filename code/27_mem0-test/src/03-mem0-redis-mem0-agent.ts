...
const REDIS_HOST = envVars.REDIS_HOST ?? 'localhost';
const REDIS_PORT = Number(envVars.REDIS_PORT ?? 6379);
const REDIS_DB = Number(envVars.REDIS_DB ?? 0);
const MEMORY_TTL = Number(envVars.MEMORY_TTL_SECONDS ?? 1800);
const KEY_PREFIX = envVars.MEMORY_KEY_PREFIX ?? 'agent:short_memory';

const USER_ID = envVars.MEM0_USER_ID ?? 'demo_user_001';
const SESSION_ID = 'session_002';
const MEM0_TOP_K = Number(envVars.MEM0_TOP_K ?? 5);
...
function messagesForRedis(messages: any[]) {
...
class Mem0MemoryStore {
  ...
  buildSystemMessage({ user, session }: { user: any[]; session: any[] }) {
    const blocks = [];
    if (user.length) {
      blocks.push(`【用户长期记忆】\n${user.map((m: any) => `- ${m.memory}`).join('\n')}`);
    }
    if (session.length) {
      blocks.push(`【当前会话记忆】\n${session.map((m: any) => `- ${m.memory}`).join('\n')}`);
    }
    if (!blocks.length) return null;
    return new SystemMessage(`${blocks.join('\n\n')}\n\n请结合以上记忆回答，勿编造。`);
  }

  async classifyAndPersist(userText: string, assistantText: string) {
    const turn = [
...
async function invokeWithMemory(
  agent: any,
  redisStore: RedisMessageStore,
  mem0Store: Mem0MemoryStore,
  sessionId: string,
  userText: string
) {
...
if (!envVars.MEM0_API_KEY || !envVars.QWEN_API_KEY) {
  console.error('需要 MEM0_API_KEY 与 QWEN_API_KEY');
  process.exit(1);
}

const redis = new Redis({ host: REDIS_HOST, port: REDIS_PORT, db: REDIS_DB });
const mem0 = new MemoryClient({ apiKey: envVars.MEM0_API_KEY });
...
const model = new ChatOpenAI({ model: envVars.QWEN_MODEL_NAME, ...llmOpts });
