...
async function addUserMemory(client: MemoryClient) {
...
async function searchUserMemory(client: MemoryClient) {
...
async function addSessionMemory(client: MemoryClient) {
  const messages: Message[] = [
...
async function searchSessionMemory(client: MemoryClient) {
...
async function addAgentMemory(client: MemoryClient) {
  const messages: Message[] = [
...
async function searchAgentMemory(client: MemoryClient) {
...
async function main() {
  if (!envVars.MEM0_API_KEY) {
    console.error('缺少 MEM0_API_KEY');
    process.exit(1);
  }

  const client = new MemoryClient({
    apiKey: envVars.MEM0_API_KEY,
  });
