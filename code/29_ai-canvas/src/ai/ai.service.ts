...
export class AiService {
  ...
  constructor(
    ...
    this.client = new MultiModalConversation(
      new Configuration({
        apiKey: this.config.getOrThrow('QWEN_API_KEY'),
      })
    );
  }
  ...
  async createImage(dto: ImageDto): Promise<ImageRecord> {
    ...
    const resultUrl = inputImageUrl
      ? await this.generateByEdit(
          [{ text: dto.prompt }, { image: this.ossService.resolveReadableUrl(inputImageUrl) }],
          options
        )
  ...
  getUploadSignature(hash: string, ext?: string) {
    return this.ossService.createUploadPolicy(hash, ext);
  }

  /**
   * @description 通过编辑图片生成新图片
   */
