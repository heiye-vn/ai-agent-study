...
const friendSchema = z.object({
  ...
  birth_date: z
    .string()
    .describe(
      `出生日期,格式:YYYY-MM-DD。如果无法确定具体日期,请根据当前日期 ${new Date().toISOString().split('T')[0]} 和提供的年龄信息估算一个合理的日期`
    ),
...
async function extractAndInsert(text: string) {
  ...
  try {
    // 切换到 hello 数据库
    await connection.query(`USE output_parser_test;`); // 使用 AI 提取结构化信息
    ...
       - 姓名：提取文本中的人名
       - 性别：提取性别信息（男/女）
       - 出生日期：如果能找到具体日期最好，否则根据当前日期 ${new Date().toISOString().split('T')[0]} 和提供的年龄信息估算一个合理的日期（格式：YYYY-MM-DD）
       - 公司：提取公司名称
       - 职位：提取职位/头衔信息
       - 手机号：提取手机号码
       - 微信号：提取微信号
    ...
    const [insertResult] = (await connection.query(insertSql, [values])) as [
