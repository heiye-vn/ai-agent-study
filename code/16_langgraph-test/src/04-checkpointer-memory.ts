...
// 每跑一轮图，给 【当前会话】访问次数 +1，并更新会话信息
function recordVisit(state: IState) {
  const visitCount = state.visitCount + 1;
  const message =
    visitCount === 1 ? '这是你在本会话里第 1 次进入。' : `这是你在本会话里第 ${visitCount} 次进入`;
...
const user2 = { configurable: { thread_id: '用户-小王' } };
