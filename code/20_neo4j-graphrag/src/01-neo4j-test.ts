...
// 连接信息（和 docker-compose 中的一致）
const dirver = neo4j.driver('bolt://localhost:7687', neo4j.auth.basic('neo4j', '12345678'));

// 获取会话
const session = dirver.session();
...
async function createRelation() {
  await session.run(`
        MATCH (p:Product {name: "珍珠奶茶"}), (i:Ingredient {name: "奶茶"})
        CREATE (p)-[:包含]->(i)
    `);
  console.log('关系创建成功');
}
